...
- `npm run build`
- Khi thêm tool mới:
- cập nhật `LeftTab` trong `src/App.tsx`
- thêm label hiển thị trong `getToolLabel`
- quyết định tool đó có thuộc `DOCUMENT_TOOL_TABS` hay không
- bổ sung prompt xử lý trong `src/components/ToolView.tsx`
